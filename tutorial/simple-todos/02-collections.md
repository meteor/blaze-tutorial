...
For our collection to work, you need to import it in the server so it sets up a skeletal structure. 

You can either use `import "/imports/api/TasksCollection"` or `import { TasksCollection } from "/imports/api/TasksCollection"` if you are going to use the same file, but make sure it is imported.
